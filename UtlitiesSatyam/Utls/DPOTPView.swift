...
@IBDesignable open class DPOTPView: UIView {
    ...
    @IBInspectable open dynamic var count: Int = 5 {
        didSet {
            if isLoaded {
                isLoaded = false
    ...
    @IBInspectable open dynamic var fontTextField: UIFont = UIFont.systemFont(ofSize: 18)
    ...
    func initialization() {
        ...
        for i in 1 ... count {
            ...
            textField.frame = CGRect(x:(CGFloat(i-1) * sizeTextField) + (CGFloat(i) * spacing/2) + (CGFloat(i-1) * spacing/2)  , y: (self.bounds.height - sizeTextField)/2 , width: 50, height: 50)
